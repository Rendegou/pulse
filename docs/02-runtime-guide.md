...
函数存在不等于被调用，被调用也不等于结果被消费。当前代码里插值链是断的：

```text
onMessage 收到 cursor
  → 仍直接改 s.x/s.y（没有样本进入 s.buf）
  → draw() 从未调用 samplePosition
  → ctx.arc 用的还是 s.x/s.y 换算出的坐标
```

L1 要让你亲手接通“缓冲写入 → 插值采样 → 绘制使用”这条链；仅给 samplePosition 写测试不能验证这条连接。
